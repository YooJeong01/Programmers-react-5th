...
function Remember() {
  ...
  // useRef() 훅 : 메모이제이션, 외부에 데이터를 기억해서 저장/읽기 용도로 사용
  // 현재(current) 기억된 값이 변경되더라도 기억은 하지만, 리-렌더 요청은 안 함
  const messageRef = useRef('메시지를 기억합니다.');
  const reRender = useState(0)[1]; //set함수만 뽑아냄
  ...
  const hnadleStateChange = ({target:{value}}:{target:{value:string}}) => {
    setMessage(value);
  }
  ...
  return (
    <main className={S.container}>
      ...
      <div className={S.description}>
        ...
        <p>어떻게 해야 리-렌더링 요청없이 메시지를 기억할 수 있을까?</p>
      </div>
